...
function drawSimplex(ctx: CanvasRenderingContext2D, w: number, h: number, t: number) {
  ...
  for (let i=0; i<3; i++) {
    ...
    // Outer %
    ctx.font = `400 ${Math.round(w*0.030)}px 'IBM Plex Mono', monospace`
  ...
  ctx.font = `400 ${fh}px 'IBM Plex Mono', monospace`
...
function drawHull(ctx: CanvasRenderingContext2D, w: number, h: number, t: number) {
  ...
  ctx.font = `400 ${fl}px 'IBM Plex Mono', monospace`
  ...
  ctx.font = `400 ${fb}px 'IBM Plex Mono', monospace`
  ctx.fillStyle = 'rgba(255,255,255,0.18)'; ctx.textAlign = 'center'
