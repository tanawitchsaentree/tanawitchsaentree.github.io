...
const PRINCIPLES = [
  {
    ...
    body: 'What scales across operators, configurators, and underwriters is clarity about what changes and what doesn\'t. Visual flexibility is fine. Behavioral inconsistency is not. The confidence gate, the review card structure, and the fallback queue SLA are the contract. The color of a confidence badge is not.',
  },
  {
  ...
  },
  {
    ...
    body: 'An error asks you to fix something, while uncertainty asks you to decide something, and the entire confidence gate rests on that distinction. Treating low confidence as an error produces a blame loop, with operators looking for who broke it. Treating it as a handoff state produces a decision loop instead, where operators own the call.',
  },
  {
  ...
  },
  {
    ...
    body: 'The prompt layer is a machine-readable contract. The AI reads every classification rule, every confidence threshold, and every fallback condition in it. The design system has to be legible to the model, not just the human configurator. If a prompt is ambiguous to a person, it\'s ambiguous to the model. That ambiguity is a bug, not a feature request.',
  },
] as const
...
const UNRESOLVED = [
  {
    ...
    body: 'Every document type, whether claim, invoice, or legal notice, uses the same review threshold. A misrouted legal notice has different downstream consequences than a misrouted supplier invoice. The threshold should adapt to document type, historical accuracy rates, and downstream cost of error. It doesn\'t, because that calibration requires historical volume per category that didn\'t exist when we shipped. The system works, but it\'s wrong for everyone in slightly different ways.',
  },
  {
    heading: 'Prompt versioning has no A/B layer',
    body: 'Configurators iterate one version at a time: publish v3, observe, then publish v4. There\'s no way to run two competing prompt strategies against the same document type at once and compare outcomes. When two configurators disagree about how a document type should be classified, the only resolution path is to try one, wait, then try the other. That\'s slow, and it means the person who publishes second wins regardless of whether their prompt is actually better.',
  },
  {
    heading: 'The confidently wrong failure mode is still undetected',
    body: 'The fallback system catches uncertainty the AI knows about, such as low confidence or an unrecognized document type. It does not catch the AI being confidently wrong. A document classified at 0.92 to the wrong category routes straight through all three layers without triggering any review. That\'s the harder problem. Solving it requires either post-hoc accuracy auditing, which is expensive and slow, or a fundamentally different approach to how the model communicates its own reliability. I don\'t have a clean answer for it yet.',
  },
] as const
    ...
    >
      ...
      >
        ...
        <img
          ...
          alt="System overview showing three interdependent layers"
