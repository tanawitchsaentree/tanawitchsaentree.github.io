...
const SCREEN_CSS = `
/* segmented toggle */
.vk-root{position:absolute;inset:0;display:flex;flex-direction:column;padding:16px 15px;font-family:'DM Sans',sans-serif;color:${V.color.ink}}
.vk-seg{position:relative;display:flex;background:${V.color.paper2};border-radius:11px;padding:3px;margin-bottom:18px}
.vk-seg button{flex:1;position:relative;z-index:2;border:0;background:none;font-family:'DM Sans',sans-serif;font-weight:600;font-size:11px;color:${V.color.muted};padding:8px 0;cursor:pointer;transition:color .25s ${V.ease.expo};display:flex;align-items:center;justify-content:center;gap:5px}
.vk-pill{position:absolute;top:3px;bottom:3px;left:3px;width:calc(50% - 3px);background:${V.color.white};border-radius:8px;box-shadow:0 2px 6px ${V.alpha.ink12};transition:transform ${V.motion.durationFast} ${V.ease.spring}}
.vk-root[data-state="shipped"] .vk-pill{transform:translateX(100%)}
.vk-root[data-state="rejected"] button[data-s="rejected"]{color:${V.color.dangerRed}}
.vk-root[data-state="shipped"] button[data-s="shipped"]{color:${V.color.limeDeep}}
/* body states */
...
.vk-kicker{font-family:'Space Mono',monospace;font-size:8.5px;letter-spacing:.14em;text-transform:uppercase;color:${V.color.muted};margin-bottom:auto}
...
.vk-den{font-family:'Bricolage Grotesque',sans-serif;font-weight:600;font-size:18px;color:${V.color.muted}}
.vk-label{font-family:'Fraunces',serif;font-style:italic;font-size:17px;text-align:center;margin-top:4px}
.vk-base{font-family:'Space Mono',monospace;font-size:8.5px;color:${V.color.muted};text-align:center;margin-top:8px}
.vk-state-rej .vk-num{color:${V.color.dangerRed}}.vk-state-rej .vk-label{color:${V.color.dangerRed}}
.vk-state-shp .vk-num{color:${V.color.ink}}.vk-state-shp .vk-label{color:${V.color.limeDeep}}
.vk-flag{margin-top:auto;display:flex;gap:8px;background:${V.color.dangerBg};border:1px solid ${V.alpha.danger25};border-radius:11px;padding:11px;font-size:10px;color:${V.color.dangerRed};line-height:1.4}
.vk-flag svg{flex:none;margin-top:1px}
.vk-ok{margin-top:auto;display:flex;gap:8px;background:${V.color.paper2};border-radius:11px;padding:11px;font-size:10px;color:${V.color.inkSoft};line-height:1.4}
.vk-ok svg{flex:none;margin-top:1px;color:${V.color.limeDeep}}
.vk-ok b{color:${V.color.ink};font-weight:700}
.vk-band{height:7px;border-radius:99px;background:${V.color.paper2};overflow:hidden;margin:14px 2px 0}
.vk-band i{display:block;height:100%;width:82%;border-radius:99px;background:linear-gradient(90deg,${V.color.lime},${V.color.limeDeep})}
/* ghost finger */
.vk-finger{position:absolute;top:0;left:0;width:28px;height:28px;z-index:20;pointer-events:none;border-radius:50%;background:radial-gradient(circle at 38% 34%,${V.alpha.ink42},${V.alpha.ink28});border:1.5px solid ${V.alpha.white80};box-shadow:0 4px 14px ${V.alpha.ink30};transform:translate(120px,90px);transition:transform ${_ms} ${V.ease.glide};opacity:0}
...
.vk-ripple{position:absolute;top:0;left:0;width:28px;height:28px;z-index:19;pointer-events:none;border-radius:50%;border:2px solid ${V.color.ink};opacity:0;transform:translate(120px,90px) scale(.3)}
