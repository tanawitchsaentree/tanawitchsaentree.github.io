...
const DOCS: Doc[] = [
  ...
  },
  {
    ...
    aiChoice: 'Claims — Standard', alternative: 'Claims — Complex',
  ...
  },
  {
    ...
    aiChoice: 'Medical Claims', alternative: 'Pre-authorisation',
...
function ReviewCard({ doc }: { doc: Doc }) {
  ...
  return (
    ...
    >
      <div
        className="mt-2 mb-3 rounded-xl border p-5"
        style={{
          background: 'var(--bg)',
      ...
      >
        <div className="flex items-start justify-between gap-4 mb-4">
          <div>
            <p className="font-mono text-[var(--type-xs)] uppercase tracking-[0.08em] text-[var(--fg-subtle)] mb-1">
              AI reading
            </p>
            <p className="font-mono text-[var(--type-base)] font-medium text-[var(--fg)]">
              {doc.aiChoice}
            </p>
          </div>
          <div className="text-right">
            ...
            <p className="font-mono text-[var(--type-base)] font-medium" style={{ color: '#dc2626' }}>
        ...
        <div className="flex items-center gap-2 mb-4">
          <span className="font-mono text-[var(--type-xs)] text-[var(--fg-subtle)] uppercase tracking-[0.06em]">vs.</span>
          <span
            className="font-mono text-[var(--type-xs)] text-[var(--fg-muted)] px-2 py-1 rounded border"
        ...
        <p className="text-[var(--type-sm)] leading-[1.65] text-[var(--fg-muted)] mb-5">
          {doc.reason}
        </p>
          ...
          {!action ? (
            <motion.div key="actions" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex gap-2">
              <button
                ...
                className="flex-1 font-mono text-[var(--type-xs)] uppercase tracking-[0.1em] py-2.5 rounded border cursor-pointer transition-colors"
            ...
            >
              {action === 'confirm' ? `✓ Routed to ${doc.aiChoice}` : `↻ Reclassified to ${doc.alternative}`}
            </motion.p>
          )}
        </AnimatePresence>
      </div>
...
function DocItem({ doc, phase, index, expanded, onToggle }: {
  ...
  return (
    <div className="border-b border-[var(--border)] last:border-b-0">
          ...
          {dispatched && (
            ...
            >
              ...
              <span
                ...
                style={isFallback ? {
                  color: '#dc2626', borderColor: 'rgba(220,38,38,0.3)', background: 'rgba(220,38,38,0.07)',
                } : {
                  color: '#16a34a', borderColor: 'rgba(22,163,74,0.3)', background: 'rgba(22,163,74,0.07)',
                }}
              >
      ...
      <AnimatePresence>
        {isFallback && dispatched && expanded && <ReviewCard doc={doc} />}
      </AnimatePresence>
    </div>
  )
}
    ...
    >
      <div className="flex items-center justify-between gap-4 px-6 py-4 border-b" style={{ borderColor: 'var(--border)' }}>
        ...
        <AnimatePresence mode="wait">
          {phase === 'idle' ? (
            <motion.button
              key="btn" type="button" onClick={dispatch}
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="font-mono text-[var(--type-xs)] uppercase tracking-[0.1em] px-3 py-1.5 rounded border cursor-pointer"
              style={{ background: 'var(--fg)', color: 'var(--bg)', borderColor: 'var(--fg)' }}
            >
              Process batch →
            </motion.button>
          ) : phase === 'done' ? (
            <motion.div key="summary" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-3">
              <span className="font-mono text-[var(--type-xs)] uppercase tracking-[0.08em]" style={{ color: '#16a34a' }}>
                {DOCS.length - fallbackCount} routed
              </span>
              <span className="font-mono text-[var(--type-xs)] text-[var(--fg-subtle)]">·</span>
              <span className="font-mono text-[var(--type-xs)] uppercase tracking-[0.08em]" style={{ color: '#dc2626' }}>
                {fallbackCount} fallback
              </span>
              <button
                type="button"
                onClick={() => { setPhase('idle'); setExpanded(null) }}
                className="font-mono text-[var(--type-xs)] uppercase tracking-[0.08em] text-[var(--fg-subtle)] hover:text-[var(--fg)] cursor-pointer transition-colors"
              >
                Reset
              </button>
            </motion.div>
          ) : (
            <motion.span key="processing" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
      ...
      <div className="px-6 py-1">
        {DOCS.map((doc, i) => (
          <DocItem
            key={doc.id} doc={doc} phase={phase} index={i}
